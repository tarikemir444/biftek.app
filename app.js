...
// ==========================================
// ⚙️ FIREBASE YAPILANDIRMASI (GÜNCELLENDİ)
// ==========================================
...
const Auth = {
    ...
    checkRole: async (uid) => {
        ...
        if(snap.exists()) {
            ...
            if(role === 'author' || role === 'admin') document.getElementById('writer-panel-btn').style.display = 'flex';
            if(role === 'admin') {
                const btn = document.getElementById('admin-panel-btn');
                btn.style.display = 'flex';
                btn.onclick = () => { UI.showSection('admin'); AdminPanel.load(); };
            }
        }
    },
    submitApp: async () => {
    ...
    load: async (cat) => {
        ...
        try {
            ...
            snap.forEach(d => {
                ...
                card.innerHTML = `
                    <img src="${data.cover || 'https://picsum.photos/seed/'+d.id+'/400/200'}" class="card-img" loading="lazy">
                    <div class="card-body">
...
const WriterPanel = {
    init: () => {
        ...
        document.getElementById('wp-ai-image').onclick = () => {
            ...
            // İçerikle alakalı daha spesifik görsel önerisi (Unsplash source mantığı)
            document.getElementById('wp-cover').value = `https://source.unsplash.com/800x600/?${encodeURIComponent(cat)},technology,science`;
    ...
    importHTML: (input) => {
        if(input.files && input.files[0]) {
            const reader = new FileReader();
            reader.onload = (e) => {
                // Basit HTML temizleme ve metne çevirme (Gelişmiş versiyonda DOMParser kullanılabilir)
...
const Games = {
    ...
    score: 0,
    timer: null,
    startDate: new Date().toDateString(), // Günlük skor için tarih anahtarı

    init: () => {
        this.switchTab('sudoku'); // Varsayılan
    ...
    // --- SUDOKU LOGIC (Basitleştirilmiş 9x9) ---
    initSudoku: () => {
    ...
    checkSudoku: () => {
        ...
        cells.forEach((c, i) => {
            if(!c.classList.contains('revealed')) {
                if(parseInt(c.textContent) === this.sudokuSolution[i]) { c.style.background = '#34C759'; c.style.color = 'white'; this.score += 10; }
                else { c.style.background = '#FF3B30'; correct = false; }
            }
        });
        if(correct) { this.saveScore('Sudoku', this.score); UI.showToast(`Tebrikler! Skor: ${this.score}`); }
    ...
    initLingo: () => {
        ...
        board.innerHTML = '';
        for(let i=0; i<30; i++) { // 6 deneme x 5 harf
    ...
    checkLingo: () => {
        ...
        else { this.lingoAttempts++; input.value = ''; if(this.lingoAttempts >= 6) { UI.showToast("Kaybettin! Kelime: " + this.targetWord); } }
    },
        ...
        for(let i=0; i<100; i++) {
            ...
            c.onclick = () => {
                ...
                if(this.mines.includes(i)) {
                    ...
                    this.score += 5;
                    // Basit komşu sayısı hesabı yerine şimdilik boş bırakıyoruz
                }
            };
            board.appendChild(c);
        }
    ...
    saveScore: async (game, score) => {
        ...
        try {
            await addDoc(collection(db, "scores"), {
                uid: currentUser.uid, name: currentUser.displayName, game, score, date: today, timestamp: serverTimestamp()
            });
    ...
    loadLeaderboard: async (game) => {
        const today = new Date().toDateString();
        document.getElementById('lb-date').textContent = today;
        const list = document.getElementById('lb-list');
        list.innerHTML = '<div style="text-align:center;">Yükleniyor...</div>';
        
        try {
            // Günlük skorları çek
            const q = query(collection(db, "scores"), where("game", "==", game), where("date", "==", today), orderBy("score", "desc"), limit(5));
            const snap = await getDocs(q);
            list.innerHTML = '';
...
const Chatbot = {
    ...
    init: () => {
        document.getElementById('chatbot-toggle').onclick = () => {
            ...
            if(elements.chatbotWindow.classList.contains('open')) {
                // Kullanıcı Yazar Panelindeyse modu değiştir
    ...
    send: () => {
        ...
        setTimeout(() => {
            let reply = "";
            if(this.mode === 'writer') {
                // Yazar Modu: Trend simülasyonu
                const trends = ["Yapay Zeka Regülasyonları", "Mars'ta Su İzleri", "Kuantum İnternet", "CRISPR Tedavisi"];
                const randomTrend = trends[Math.floor(Math.random()*trends.length)];
                reply = `Harika bir seçim! Güncel verilere göre "**${randomTrend}**" konusu çok konuşuluyor. Başlık önerim: "Geleceği Şekillendiren ${randomTrend}: Bilmeniz Gerekenler". İçerik taslağı oluşturmamı ister misiniz?`;
            } else {
                // Kullanıcı Modu: Haber Önerisi
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // URL Parametre ile Haber Açma
    const params = new URLSearchParams(window.location.search);
    if(params.has('id')) {
        // Basit bir fetch ile haberi bulup açma (Detaylı implementasyon için ID ile sorgu gerekir)
        // Şimdilik kullanıcı ana sayfadan tıklasın.
    }
